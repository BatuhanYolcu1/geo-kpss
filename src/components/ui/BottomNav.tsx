...
export default function BottomNav() {
    ...
    return (
        <nav className="fixed bottom-0 left-0 right-0 z-[999] md:hidden">
            ...
            <div className="bg-white border-t border-[#abb4ac]/40 backdrop-blur-xl px-2 pb-[env(safe-area-inset-bottom)]">
                        ...
                        return (
                            <Link
                                ...
                                className={`flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-all duration-200 min-w-[56px] ${isActive
                                        ? 'text-[#386948]'
                                        : 'text-[#59615a] active:text-[#2c342e]'
                                    }`}
                            >
                                <div className={`relative ${isActive ? 'scale-110' : ''} transition-transform`}>
                                    <Icon size={22} strokeWidth={isActive ? 2.5 : 2} />
                                    {isActive && (
                                        <div className="absolute -inset-2 bg-[#386948]/10 rounded-xl -z-10" />
                                    )}
                                </div>
                                <span className={`text-[10px] font-bold ${isActive ? 'text-[#386948]' : ''}`}>
                                    {item.label}
                                </span>
                            </Link>
                        );
